...
class WxPay{
     ...
     */
    public function payOrder($order_no, $txnTime, $txnAmt,$notify_url){
     ...
     * @param string $batch_no  退款单号
     * @param double $money 交易金额 单位为元
     * @return bool 成功时返回
     * @throws Exception
     * @throws WxPayException
     */
    public function refund($transaction_id,$batch_no, $money){
        $input = new WxPayRefund();
        $input->SetTransaction_id($transaction_id);
        $input->SetTotal_fee(intval($money*100));
        $input->SetRefund_fee(intval($money*100));
...
//        var_dump($result);
    ...
    /**
     * 企业付款给用户
     * @param string $order_no 商户订单
     * @param string $openid  openid
     * @param int $money 交易金额单位分
     * @return array
     * @throws Exception
     */
    public function payment($order_no, $openid, $money){
        $input = new WxPayment();
        $input->setTradeNo($order_no);
        $input->setOpenid($openid);
        $input->setAmount($money);
        $input->setDesc("提现");

        $result = WxPayApi::payment($input);
...
class AliPay{
    ...
    public function __construct(){
        ...
        self::$PAYMENT_URL = C("HOST")."index.php/Admin/Pay/aliPayPaymentCallBack";         //付款回调
     ...
     * @return string
     */
    public function getRequestParam($order_no, $money, $content,$notify_url){
        $request = [
            "partner"=>PARTNER,
            "seller_id"=>SELLER_ID,
...
//            "return_url" => "m.alipay.com"
     ...
     * @param $account
     * @param $account_name
     * @return string 支付宝提现URL
     */
    public function payment($order_no, $money, $account, $account_name){
        $result = $this->aliPayApi->payment($order_no,$money,$account,$account_name,self::$PAYMENT_URL);
        return $result;
    }
